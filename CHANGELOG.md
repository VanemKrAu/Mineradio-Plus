...
### 新增
- **热评卡片壁纸视频导入** — 从 WE 壁纸库选择壁纸，PKG 解包提取视频/纹理，裁切后设为每日热评背景
- **PKG 解包提取** — 集成 RePKG，自动解包 scene.pkg 提取 MP4 视频或单层纹理图片
- **年龄分级筛选** — WE 壁纸库工具栏新增下拉筛选（全部/全年龄/可疑/成人），选择持久化
- **托盘功能增强** — 当前歌曲信息、播放控制、音量调节、开机自启开关
- **SMTC 系统媒体控件** — 专辑封面 + 进度条 + 媒体键 + 锁屏同步 + 快进快退

### 变更
- 项目更名为 Mineradio+，版本号 2.0.2-1
