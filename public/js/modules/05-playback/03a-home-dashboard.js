...
function homeDashboardReleaseVideoSource(removeElement) {
  ...
  // Clean up WE wallpaper engine preview image
  var weBg = document.querySelector('#empty-home .home-dashboard-we-bg');
  if (weBg && weBg.parentNode) weBg.parentNode.removeChild(weBg);
...
async function homeDashboardAttachVideo() {
  ...
  // WE wallpaper engine preview
  if (meta.type === 'wallpaper-engine' && meta.previewUrl) {
    var card = document.querySelector('#empty-home .daily-review-card');
    if (!card) return;
    var existing = card.querySelector('.home-dashboard-we-bg');
    if (existing && existing.getAttribute('data-preview-url') === meta.previewUrl) return;
    homeDashboardReleaseVideoSource(true);
    var img = document.createElement('img');
    img.className = 'home-dashboard-video home-dashboard-we-bg';
    img.setAttribute('data-preview-url', meta.previewUrl);
    img.setAttribute('aria-hidden', 'true');
    img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;border-radius:inherit';
