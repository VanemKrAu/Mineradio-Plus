...
function extractWallpaperTexture(folderPath) {
  ...
  if (!scene.ok) {
    console.warn('[Wallpaper] extractWallpaperTexture: scene extraction failed:', scene.error || 'unknown');
    return '';
  }
  ...
  if (!bestPath) console.warn('[Wallpaper] extractWallpaperTexture: no textures found, count=', scene.textures.length);
  return bestPath;
}
