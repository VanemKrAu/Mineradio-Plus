...
const APP_USER_MODEL_ID = 'com.mineradio-plus.desktop';
...
async function openKugouMusicLoginWindow(owner) {
  var cookieSession = session.fromPartition(KUGOU_LOGIN_PARTITION);
  var initialCookie = await readKugouLoginCookieHeader(cookieSession);
  ...
  return new Promise(function(resolve) {
    ...
    var loginWindow = new BrowserWindow({
      ...
      title: '酷狗音乐登录', backgroundColor: '#111111',
      icon: APP_ICON_ICO,
      webPreferences: { partition: KUGOU_LOGIN_PARTITION, contextIsolation: true, nodeIntegration: false, sandbox: true }
    });
    ...
    loginWindow.on('closed', async function() {
      ...
      try {
        var cookie = await readKugouLoginCookieHeader(cookieSession);
        resolve(kugouCookieHasLogin(cookie) ? { ok: true, cookie: cookie } : { ok: false, cancelled: true, message: '酷狗登录窗口已关闭' });
      } catch(e) { resolve({ ok: false, error: e.message || '酷狗登录窗口已关闭' }); }
    });
    pollTimer = setInterval(checkCookies, 1500);
    loginWindow.loadURL(KUGOU_LOGIN_URL).catch(function(e) { finish({ ok: false, error: e.message }); });
  });
}
async function clearKugouMusicLoginSession() {
  try {
    var ses = session.fromPartition(KUGOU_LOGIN_PARTITION);
...
ipcMain.handle('mineradio-wallpaper-scan', async (_event, rootPaths) => {
  try {
    console.log('[WallpaperScan] roots:', JSON.stringify(rootPaths));
    const list = wallpaperScanner.scanLibraries(rootPaths);
    console.log('[WallpaperScan] found:', list.length, 'wallpapers');
    return { ok: true, wallpapers: list };
  } catch (e) {
    console.error('[WallpaperScan] error:', e);
...
ipcMain.handle('mineradio-wallpaper-read-file', async (_event, filePath) => {
  try {
    ...
    console.error('[WallpaperReadFile] FAILED path:', filePath, 'error:', e.code, e.message);
...
ipcMain.handle('mineradio-wallpaper-clear-cache', async () => {
  try {
    var roots = wallpaperScanner.autoDetectRoots();
    var result = wallpaperScanner.clearWallpaperCache(roots);
    return { ok: true, cleaned: result.cleaned, bytes: result.bytes };
  } catch(e) { return { ok: false, error: e.message }; }
});
